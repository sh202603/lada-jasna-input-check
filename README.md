...
| 項目 | 判定 | 根拠 |
|---|---|---|
| コーデックが h264/hevc/vp9/av1 以外 | **FAIL** | NVDEC でのデコードが前提のため非対応です |
| `color_space` が bt709/smpte170m/bt470bg 以外 | **FAIL** | BT.709/BT.601 以外は `UnsupportedColorspaceError` で停止します |
| `color_space` が未設定 | WARN | 解像度から BT.709/601 を推定します（外れると色がずれます） |
| 負の PTS のパケット | WARN | NVDEC でのデコード時に黙って破棄されます（先頭欠け） |
...
```
=== sample.mp4 ===
  メタデータ: mp4 / h264 / 1920x1080 / yuv420p / 29.97fps / 0:12:34 / 音声: aac
  [WARN] (lada-ex) 負の開始 PTS (first_pts=-2002)。...
  [FAIL] (jasna) color_space=bt2020nc — jasna は BT.709/BT.601 のみ対応 ...
  判定: lada-ex → 注意 / jasna → NG
...
- スコープのラベルは、共通項目には付かず、ツール固有の項目には `(lada-ex)` / `(jasna)` が付きます。
...
| jasna_reencode | jasna 非対応のコーデック | `hevc_nvenc` で再エンコード。インターレース検出時は `-vf bwdif` を自動付与 | 再エンコード |
| color_convert | BT.2020/HDR などの色空間 | zscale + tonemap で BT.709 化（SDR 素材向けの軽量版も注記） | 再エンコード |
...
- 音声はコーデック名の表示のみです。jasna はコンテナ非互換の音声を自動で AAC に再エンコードするため、事前チェックは不要です。
- 判定基準は lada-ex / jasna の調査時点の仕様に基づいています。ツール側の仕様変更時は、スクリプト冒頭の定数の見直しが必要です。
